...
Complete API documentation for the hell fire progression system, including the ProgressionManager, Player leveling mechanics, and world unlock logic.
...
function saveGame() {
  ...
  localStorage.setItem('hell_fire_save', JSON.stringify(saveData));
}

// Load
function loadGame() {
  const raw = localStorage.getItem('hell_fire_save');
  if (raw) {
