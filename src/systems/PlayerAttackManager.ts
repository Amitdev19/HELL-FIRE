...
export class PlayerAttackManager {
  ...
  setupPlayerAttack(inventoryUI: { getIsVisible: () => boolean }, levelUpUI: { getIsVisible: () => boolean }): void {
    this.scene.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      // Ignore taps on the mobile touch UI (buttons/joystick) so they don't also shoot.
      if (InputState.uiActive) return;
  ...
  /**
   * Fire an attack toward a specific angle. If `aimAngle` is undefined, the
   * manager auto-aims at the nearest active enemy; if none, it falls back to
   * the player's current facing direction. Used by both mouse aim and the
   * mobile touch "Attack" button.
   */
  fireFromAngle(aimAngle?: number): void {
    ...
    if (angle === undefined) {
      ...
      if (enemy) {
        ...
        angle = this.facingToAngle(this.player.getFacingDirection());
      }
    }
  ...
  private facingToAngle(facing: string): number {
    switch (facing) {
      ...
      case 'north_east': return -Math.PI / 4;
      case 'south_east': return Math.PI / 4;
      case 'south_west': return (3 * Math.PI) / 4;
      case 'north_west': return (-3 * Math.PI) / 4;
