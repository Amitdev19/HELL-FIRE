...
[![Relay](https://img.shields.io/badge/Co--op-Self--hosted%20WebSocket-blue)](https://github.com/devils_call)

Co-op multiplayer system for HELL FIRE, backed by our **own self-hosted
WebSocket relay server** (`/server`). One player hosts, another joins as a
"helper" with 75% of the host's stats. No third-party P2P service required —
run `npm run server` and the game connects to `ws://<host>:3001`.
