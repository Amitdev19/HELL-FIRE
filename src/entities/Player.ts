...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  canDodge(): boolean {
    return this.dodgeCooldown <= 0 && !this.isDodging;
  }

  private handleMovement(): void {
    // Read keyboard state into the shared input state (touch overrides this).
    let kbx = 0;
    let kby = 0;
    if (this.cursors?.left.isDown || this.wasd?.A.isDown) kbx = -1;
    else if (this.cursors?.right.isDown || this.wasd?.D.isDown) kbx = 1;
    if (this.cursors?.up.isDown || this.wasd?.W.isDown) kby = -1;
    else if (this.cursors?.down.isDown || this.wasd?.S.isDown) kby = 1;
    InputState.setKeyboardMove(kbx, kby);

    let velocityX = InputState.moveX;
    let velocityY = InputState.moveY;
