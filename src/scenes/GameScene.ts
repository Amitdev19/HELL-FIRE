...
import { InputState, isTouchDevice } from '../systems/InputState';
...
export class GameScene extends BaseScene {
  ...
  private createUI(): void {
    ...
    this.settingsUI.setControlModeChangeCallback((mode: 'pc' | 'mobile') => {
  ...
  private toggleInventoryFromTouch(): void {
    if (this.settingsUI.getIsVisible() || this.levelUpUI.getIsVisible()) return;
  ...
  private setupMobileControls(): void {
    this.applyControlMode(SettingsManager.getControlMode());
  }

  private applyControlMode(mode: 'pc' | 'mobile'): void {
    this.scale.off('resize', this.onResize, this);
    this.mobileControls?.destroy();
    this.mobileControls = null;

    if (mode === 'mobile') {
      this.mobileControls = new MobileControls(this, () => this.toggleInventoryFromTouch());
  ...
  update(time: number, delta: number): void {
    ...
    // Mobile touch intents (auto-aim attack, dodge, inventory toggle)
    if (this.mobileControls) {
      if (InputState.consumeInventoryToggle()) {
        this.toggleInventoryFromTouch();
      }
      if (InputState.consumeAttack()) {
        this.playerAttackManager.fireFromAngle(InputState.aimAngle);
      }
      if (InputState.consumeDodge() && this.player.canDodge()) {
        this.player.dodge();
      }
    }
