...
export class GameOverScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    if (isWorldMode) {
      continueAction = () => this.scene.start('HubScene');
      this.createButton(width / 2, height * 0.75, 'Return to Hub', continueAction);

      this.createButton(width / 2, height * 0.85, 'Main Menu', () => {
        this.scene.start('MenuScene');
      });
    } else {
      continueAction = () => { this.registry.set('floor', 1); this.scene.start('GameScene'); };
      this.createButton(width / 2, height * 0.75, 'Try Again', continueAction);
    ...
    // Enter key to continue (primary action)
    if (continueAction) {
      this.input.keyboard?.on('keydown-ENTER', continueAction);
      this.input.keyboard?.on('keydown-SPACE', continueAction);

      const hint = this.add.text(width / 2, height * 0.72, '[ Press ENTER to continue ]', {
        fontSize: '11px',
        fontFamily: 'monospace',
        color: '#6b7280',
      });
