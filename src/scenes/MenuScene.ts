...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Creator credit
    const creator = this.add.text(width - 15, height - 30, 'Created by devils_call', {
      fontSize: '10px',
      fontFamily: 'Roboto Mono, Courier New, monospace',
      color: '#333333',
    });
  ...
  private createTitle(width: number, height: number): void {
    ...
    const titleGlow = this.add.text(width / 2, height * 0.28, 'HELL\nFIRE', {
      fontSize: '64px',
    ...
    // Main title
    const title = this.add.text(width / 2, height * 0.28, 'HELL\nFIRE', {
      fontSize: '64px',
      fontFamily: 'Cinzel, Georgia, serif',
      color: '#ffffff',
      align: 'center',
    ...
    const subtitleText = 'Rise Through the Inferno';
  ...
  shutdown(): void {
    ...
    // Clean up network callbacks for safety
    networkManager.clearOnPeerJoin();
  }
}
