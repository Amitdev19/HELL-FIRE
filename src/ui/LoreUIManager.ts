...
export class LoreUIManager {
  ...
  private loreKeyHandler: ((event: KeyboardEvent) => void) | null = null;
  ...
  closeModal(): void {
    if (this.loreKeyHandler) {
      this.scene.input.keyboard?.off('keydown', this.loreKeyHandler);
      this.loreKeyHandler = null;
    }
  ...
  private showLoreModal(lore: LoreEntry): void {
    ...
    // Continue prompt
    const continueText = this.scene.add.text(0, panelHeight / 2 - 35, '[ Click or press ENTER to continue ]', {
      fontSize: '12px',
      color: '#9ca3af',
      fontStyle: 'italic',
    });
    ...
    // Keyboard: Enter/E to continue, Esc to close
    ...
    this.loreKeyHandler = keyHandler;
  }
